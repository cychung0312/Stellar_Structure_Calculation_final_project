...
#                  [20.0, 8.5, 13.0, 40.0])
    ...
    m_eps = 1e-8 * M_star # Small offset from center to avoid 1/r singularity
...
def run_solver(M_star, init_guess, X, Y, Z):
    ...
    opts = {'xtol': 1e-8} if method_choice == 'hybr' else {'ftol': 1e-8, 'xtol': 1e-8}
    
    result = root(shootf, init_guess, args=(M_star, X, Y, Z, 0.5), 
...
if final_params is not None:
    ...
    sol_c = solve_ivp(derivs, (m_eps, m_fit), y_c_start, args=(X, Y, Z), method='Radau', rtol=1e-8)
    
    y_s_start = load2(L_f, R_f, M_star, X, Y, Z)
    sol_s = solve_ivp(derivs, (M_star, m_fit), y_s_start, args=(X, Y, Z), method='Radau', rtol=1e-8)
